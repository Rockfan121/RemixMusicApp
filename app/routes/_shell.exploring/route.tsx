...
const USER_ID = "u/669b02cc904a229c1a956b3b";
export async function loader() {
	const res = await fetch(
		`https://openwhyd.org/${USER_ID}/playlists?format=json&limit=100`,
	);
	const resJson = await res.json();
	const firstPlaylistId = (await resJson)[0].id;
	const userNameRes = await fetch(
		`https://openwhyd.org/${USER_ID}/playlist/${firstPlaylistId}?format=json&limit=100`,
	);
	return json({
		res: await resJson,
...
export default function Exploring() {
	...
	return (
		<>
			<div className="flex w-60 max-w-sm items-center space-x-1 mx-6 mb-10">
