...
function makeRequest(params: Record<string, string> = {}) {
	const url = new URL(`http://localhost/tracks/${USER_ID}/stream`);
...
describe("user-stream loader", () => {
	...
	it("passes the after parameter to the stream endpoint", async () => {
		let receivedAfter: string | null = null;
		server.use(
			http.get("https://openwhyd.org/api/user/:userId", () =>
				HttpResponse.json(USER_INFO),
			),
			http.get("https://openwhyd.org/stream", ({ request }) => {
	...
	it("returns empty TRACKS and hasMore=false when the stream endpoint fails", async () => {
		server.use(
			http.get("https://openwhyd.org/api/user/:userId", () =>
				HttpResponse.json(USER_INFO),
			),
			http.get(
				"https://openwhyd.org/stream",
